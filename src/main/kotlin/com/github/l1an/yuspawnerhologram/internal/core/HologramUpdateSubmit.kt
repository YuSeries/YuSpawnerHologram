...
object HologramUpdateSubmit {
                ...
                for (i in 0 until lines.size()) {
                    ...
                    if (line is TextHologramLine) {
                        ...
                        when (hologramInfo.type) {
                            COOLDOWN -> {
                                line.text = hologramInfo.text.replace(
                                    "%cooldown%",
                                    secondToFormat(config, spawnerManager.remainingCooldownSeconds, "durationFormat")
                                )
                            }
                            ...
                            WARMUP -> {
                                line.text = if (!spawnerManager.isOnWarmup) {
                                    ...
                                    hologramInfo.text.replace(
                                        "%warmup%",
                                        secondToFormat(config, spawnerManager.remainingWarmupSeconds, "durationFormat")
                                    )
                                }
                            }
    ...
    val activeMsg: String
        get() = Coerce.toString(config["activeMsg"]).replace("&", "§")
}